            ...
            return 0,0,0 #NADIE QUISO DESAFIAR, LA ACCIÓN SE HACE O PASA A CONTRA-ATAQUE
                ...
                if self.time == 1:
                    ...
                    if self.action != 6:
                        ...
                        if self.players_list[self.player]._Players__influence1 != block_card[0] and self.players_list[self.player]._Players__influence2 !=block_card[1] and self.players_list[self.player]._Players__influence2 !=block_card[1]:
                            print("El jugador "+str(self.player+1)+" no posee la influencia "+ names[0]+" ni " +names[1]+", da vuelta una carta")
                            self.show_card(self.player+1) #La acción se realiza porque el contraataque fue falso
                            return 0, 1
                        elif self.players_list[self.player]._Players__influence1 == block_card[0]:
                            print("El jugador "+str(self.player+1)+" posee la influencia "+names[0]+", el jugador "+str(a+1)+" da vuelta una carta") #La acción no se realiza, pierde las monedas
                            self.show_card(a+1)
                            print("El jugador "+str(self.player+1)+" devuelve su carta al mazo y saca otra")
                            ...
                            self.show_card(a+1)
                            print("El jugador "+str(self.player+1)+" devuelve su carta al mazo y saca otra")
                            return 2,self.player+1, 0
                        elif self.players_list[self.player]._Players__influence1 == block_card[1]:
                            print("El jugador "+str(self.player+1)+" posee la influencia "+names[1]+", el jugador "+str(a+1)+" da vuelta una carta") #La acción no se realiza, pierde las monedas
                            self.show_card(a+1)
                            print("El jugador "+str(self.player+1)+" devuelve su carta al mazo y saca otra")
                            return 1,self.player+1,0 
                        elif self.players_list[self.player]._Players__influence2 == block_card[1]:
                            print("El jugador "+str(self.player+1)+" posee la influencia "+names[1]+", el jugador "+str(a+1)+" da vuelta una carta") #La acción no se realiza, pierde las monedas
                            self.show_card(a+1)
                            print("El jugador "+str(self.player+1)+" devuelve su carta al mazo y saca otra")
                            return 2,self.player+1,0
